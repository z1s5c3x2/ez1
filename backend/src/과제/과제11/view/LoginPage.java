...
public class LoginPage {
	...
	public void loginMenu() {
		...
		while( MemberController.getInstance().getLoginSession() != 0 ) { // 로그인이 되어 있는 경우에만 메뉴 반복 
			...
			System.out.println("\n\n ===== ===== ===== ===== ");
			System.out.print("1.로그아웃 2.회원정보 3.글쓰기 4.글조회 선택 5. 쪽지목록보기: ");
			try {
	...
	public void boardView() {
		...
		if(ch == 4){messageSend(result.getBno());}
		
	}
	// 12. boardUpdate : 게시물 수정 
	public void boardUpdate(int b,int aut) {
		...
		BoardController.getInstance().boardUpdate(b,MemberController.getInstance().getLoginSession(),con,title);
		
	}
	// 13. boardDelete : 게시물 삭제
	public void boardDelete(int b,int aut) {
		BoardController.getInstance().boardDelete(b,MemberController.getInstance().getLoginSession());
	}
	public void messageSend(int bno)
	{
		sc.nextLine();
		System.out.println("작성할 쪽지 내용을 입력 해 주세요 >");
		String msg = sc.nextLine();
		boolean _tmp = MessageController.getInstance().sendMsg(bno, MemberController.getInstance().getLoginSession(), msg);
	...
	public void msgView()
	{
		...
		ArrayList<MessageDto> lst = MessageController.getInstance().getMsgList(MemberController.getInstance().getLoginSession());
		for(int i=0;i<lst.size();i++)
		{
			System.out.printf("보낸사람:%s   내용:%s 날짜:%s\n",lst.get(i).getMeseid(),lst.get(i).getMecontent(),lst.get(i).getMedate());
		}
		if(lst.size() == 0)
		{
			System.out.println("받은 쪽지가 없습니다");
		}
		System.out.println("1.답장 2.뒤로가기");
		int ch = sc.nextInt();
		sc.nextLine();
		if(ch ==1 )
		{
			System.out.println("받을 사람의 아이디 입력:");
			
			messageReply(sc.nextLine());
		}

		
	}
	public void messageReply(String mid)
	{
		System.out.println("답장 내용 입력");
		String con = sc.nextLine();
		boolean tmp = MessageController.getInstance().messageReply(mid,MemberController.getInstance().getLoginSession(),con);
		System.out.println("전송"+tmp);
	}
}
