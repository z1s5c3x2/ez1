...
public class BoardDao extends Dao {
	...
	private void addViewCnt(int bno,int nowcnt)
   {
      try {
         ...
         // TODO: handle exception
    	  System.out.println("조회수 업데이트 에러"+e);
	...
	public boolean boardUpdate(int bno,String Title, String con,int aut) {
		

		try {
			
			String sql = "update board set btitle = ifNULL(?,btitle), bcontent = ifNULL(?,bcontent) where bno = ? and mno = ?";
			ps = conn.prepareStatement(sql);
			...
			// TODO: handle exception
			System.out.println("게시물 수정 오류"+e);
		}
		return false;
	}
	// 13. boardDelete : 게시물 삭제
	public boolean boardDelete(int bno,int aut) {
		try {
