...
public class Test05 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// DTO -----> JSON 변환 [ * JS에게 데이터 전달하는데 ... JS는 Dto타입을 모름 ]
			...
			// 라이브러리 : jackson-annotations-2.14.2   /   jackson-core-2.14.2   /   jackson-databind-2.14.2
				// ObjectMapper 클래스 : JAVA객체를 JSON형식으로 변환해주는 함수들을 제공하는 클래스
					// .writeValueAsString( JAVA객체 ) : 해당 객체를 JSON형식의 문자열로 반환
		
		// 1. Dto --> JSON 변환해주는 변환 객체 [ - ObjectMapper  ]
		ObjectMapper mapper = new ObjectMapper();
		String jsonData = mapper.writeValueAsString(boardDto);
			// json형식으로 변환할 객체 넣기
		System.out.println("자바[String타입]에서의 JSON 모양으로 변환해주는 라이브러리[JSON] : "+jsonData);
